...
- Oracle 
- Instalace a připojení k PostgreSQL
...
```
   - následně nainstalujeme Gedit, PostgreSQL, PostGIS, QGIS a GDAL
...
sudo apt-get install -y gedit postgresql postgis qgis gdal-bin
```
...
```
   - příkazovou řádku lze uzavřít a připojit se z GUI PGAdmin4 k serverlu `localhost` s uživatelem `postgres` a heslem `heslo` (nastavené výše)

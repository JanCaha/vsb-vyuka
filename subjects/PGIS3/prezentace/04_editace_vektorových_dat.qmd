...
- aktuálně nás zajímají funkce pro tvorbu geometrií - `acpy.Point()`, `acrpy.Multipooint()`, `acpy.Polyline()`, `acpy.Polygon()`
...
- součást modulu `acrpy.da` (např. `acrpy.da.UpdateCursor`)
...
- `spatial_filter` - prostorový filtr `acrpy.Geometry` objekt
...
```python
cursor.insertRow(("hodnota 1", 23, acrpy.Point(1, 2)))
```

...
# Správa Pythonu{.center}

## [Conda]{.software}

- systém pro správu Pythonu a jeho balíčků ([informace](../../../tools/conda.qmd))
...
## Proč [Conda]{.software} a ne `venv`?

- [Python]{.software} má i nativní nástroj pro virtuální prostředí - `venv` 
- `venv` spravuje pouze Python balíčky instalované přes [pip]{.software}
- [Conda]{.software} spravuje i **kompilované systémové závislosti** (C/C++ knihovny)
- pro GIS knihovny ([GDAL]{.software}, [rasterio]{.software}, [fiona]{.software}) je to klíčové - závisí na kompilovaných knihovnách, které [pip]{.software} neumí správně nainstalovat
- [Conda]{.software} = **správa Pythonu i jeho okolí** {{< fa arrow-right >}} `venv` = pouze Python balíčky
- v praxi: pro GIS vždy preferovat [Conda]{.software}, pro čistě Python projekty (web, API) může stačit `venv`
...
def fun(vals: typing.List[float],
        round_up: bool = True) -> typing.List[int]: ...

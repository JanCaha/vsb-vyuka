...
echo "deb [signed-by=/etc/apt/trusted.gpg.d/marutter_pubkey.asc] https://cloud.r-project.org/bin/linux/ubuntu $(lsb_release -c)-cran40/" | sudo tee /etc/apt/sources.list.d/r.list
sudo apt-get update
...
echo "deb [arch=amd64] https://r2u.stat.illinois.edu/ubuntu $(lsb_release -c) main" | sudo tee /etc/apt/sources.list.d/cranapt.list
sudo apt-get update
